...
export class Chevere {
    ...
    findRefs() {
        this.refs = [...this.element.querySelectorAll("*[data-ref]")].reduce(
            (props, el) => {
                if (!el.dataset.ref)
    ...
    refreshChilds(attr, name) {
        this.childs[attr]
            .filter((node) => node.attr?.values.original.includes(name))
            .forEach((node) => {
                node.setAction();
            });
    }
    ...
    $emitSelf(data) {
        this.childs["data-on"]
            .filter((node) =>
                node.attr.some((attrs) => attrs.attribute.includes(data.name)),
            )
            .forEach((node) =>
    ...
    updateRelated(name) {
        ["data-show", "data-text"].forEach((attr) =>
            this.refreshChilds(attr, name),
        );
        this.childs["data-model"]
            .filter((node) => node.variable == name)
