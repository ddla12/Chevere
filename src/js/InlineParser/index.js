...
function parserData(str) {
     ...
     * @type {string}
     */
    var parsedStr = str.replace(pattern, function (matched) {
     ...
     * @type {object}
     */
    var sanitized = JSON.parse(parsedStr);
    /**
     ...
     * @type {ChevereObject[]}
     */
    var final = data.variables.map(function (variable) {
...
function parseClick(el, data, id) {
    ...
    var parseBool = (patterns.boolean.base.test(attr));
    var parseNum = (patterns.numeric.base.test(attr));
    ...
    if (related.text != undefined)
        setTextElements(related.text.map(function (el) { return el.element; }), check.value);
    //Set the click event
    setClick({
        el: el,
...
function setRelationsArray(list, type) {
    if (list) {
        return Array.from(list).map(function (el) {
            return {
                element: el,
                type: type
            };
